...
//   const calculateTotalPrice = (selectedItems, shippingFee = 4.0) => {
...
//           `https://backend-doancoso.onrender.com/customer/api/user?id=${userId}`
//         );
//         if (!userResponse.ok)
//           throw new Error(
//             `Error getting user information: ${userResponse.statusText}`
//           );
...
//           throw new Error("Missing user email or phone!");
//       } catch (error) {
...
//           `https://backend-doancoso.onrender.com/address/api/${userId}`
//         );
...
//           throw new Error("No valid address found for this user!");
//       } catch (error) {
...
//         "https://backend-doancoso.onrender.com/bill/api/create",
//         {
//           method: "POST",
...
//           fetch(
//             `https://backend-doancoso.onrender.com/cart/api/delete/${userId}/${item.productId}`,
//             { method: "DELETE" }
//           )
//         )
//       );
...
const COD = ({
  ...
  const handleOrderConfirmation = async () => {
    ...
    try {
      ...
      try {
        const userResponse = await fetch(
          `${API}/customer/api/user?id=${userId}`,
          { credentials: "include" }
        );
        if (!userResponse.ok)
          throw new Error(
      ...
      let selectedAddressInfo = null;
      try {
        const addressResponse = await fetch(`${API}/address/api/${userId}`, {
          credentials: "include",
        });
        if (!addressResponse.ok) throw new Error("Error getting address list");
      ...
      const response = await fetch(`${API}/bill/api/create`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(billData),
      ...
      await Promise.all(
        selectedItems.map((item) =>
          fetch(`${API}/cart/api/delete/${userId}/${item.productId}`, {
            method: "DELETE",
            credentials: "include",
          })
        )
