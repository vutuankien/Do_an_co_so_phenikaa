...
//           wishlist.map((item) => String(item.productId))
//         );
...
//   useEffect(() => {
//     fetch(`https://backend-doancoso.onrender.com/cosmetic/api/${id}`)
//       .then((res) => res.json())
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    fetch(`${API}/cosmetic/api/${id}`, { credentials: "include" })
      .then((res) => res.json())
      .then((data) => setProduct(data))
      .catch((error) => console.error("Lỗi khi tải chi tiết sản phẩm:", error));
  }, [id, API]);
  ...
  const handleLike = async (product) => {
    try {
      ...
      // refresh wishlist
      const wl = await fetch(`${API}/wishlist/api/${userUID}`, {
        credentials: "include",
      });
      setLikedProducts(
        new Set((await wl.json()).map((i) => String(i.productId)))
      );
    } catch (error) {
      console.error("Lỗi khi thêm/xóa khỏi danh sách yêu thích:", error);
    }
