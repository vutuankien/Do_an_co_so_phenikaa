...
const Qr = ({ show, onClose, selectedItems, cartItems, userId, selectedAddress, onOrderSuccess }) => {
            ...
            {!isLoading && show && (
                <div className="qr fixed inset-0 flex flex-col items-center justify-center z-[9999] p-4 bg-black bg-opacity-50">
                    <div className="boc flex flex-col items-center justify-center bg-white p-6 rounded-lg shadow-lg relative">
                        <button className="absolute top-2 right-2 text-gray-600 hover:text-gray-900 text-2xl font-bold" onClick={onClose}>
                            <FaTimes />
                        </button>
                        <img src={QRImage} alt="QR Code" className="w-64 h-64 object-cover mb-4 rounded-lg shadow-lg" />
                        <button className="px-6 py-3 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition" onClick={handleOrderConfirmation}>
                            Order Confirmation
                        </button>
                    </div>
                </div>
